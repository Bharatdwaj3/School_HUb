    ...
    <div className="min-h-screen bg-bg flex">
      <div className="hidden lg:flex w-[45%] bg-primary flex-col justify-between px-16 py-14 relative overflow-hidden">
        ...
        <div className="absolute right-0 top-0 bottom-0 w-20 flex items-center justify-center overflow-hidden pointer-events-none">
          <span className="text-[8rem] font-black text-text-inverse/4 uppercase select-none whitespace-nowrap" style={{ writingMode: 'vertical-rl', letterSpacing: '-0.05em' }}>LOADING</span>
        </div>
        ...
        <div>
          ...
          <p className="text-[10px] font-black uppercase tracking-[0.5em] text-accent/50 mb-4">Please Wait</p>
          <h1 className="text-6xl font-black text-text-inverse uppercase leading-none tracking-tight mb-4">Loading<br />Dashboard</h1>
          <p className="text-sm text-text-inverse/40 leading-relaxed max-w-xs">Fetching student records, attendance, and notices. This won&apos;t take long.</p>
        </div>
        <div className="flex gap-8">
          {[['...', 'Loading'], ['📊', 'Syncing'], ['→', 'Almost Ready']].map(([num, label]) => (
            <div key={label}>
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-8 text-center">
        ...
        <p className="text-[10px] font-black uppercase tracking-[0.5em] text-cta mb-3">Loading</p>
        <h2 className="text-3xl font-black text-primary uppercase tracking-tight mb-2">Preparing SchoolHub</h2>
        <div className="w-12 h-0.5 mx-auto mb-6" style={{ background: 'linear-gradient(90deg, transparent, var(--color-accent), transparent)' }} />
        <p className="text-sm text-text-muted leading-relaxed max-w-sm mx-auto">Retrieving latest data for students, classes, and operations. Hang tight.</p>
      </div>
    </div>
  );
}